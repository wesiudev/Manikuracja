      ...
      if (event.latLng && geocoder.current) {
        ...
        geocoder.current.geocode({ location }, (results, status) => {
          if (status === "OK" && results && results[0]) {
            const newAddress = results[0].formatted_address; // Get new address immediately

            // Use newAddress instead of previous state value
            generatePage({
              lat: location.lat,
              lng: location.lng,
              place: newAddress,
              id: removePolishPostalCodes(createLinkFromText(newAddress)),
            });
