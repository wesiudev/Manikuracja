...
export default function Logic({ slugCity }: { slugCity?: string }) {
  ...
  return (
    <div className="items-center justify-center flex flex-col mt-3 w-full">
      <div className="relative">
        ...
        <input
          ...
          className="bg-black/50 text-lg placeholder:text-white block sm:max-w-1/2 px-2 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-400"
          autoComplete="off"
        ...
        <button
          type="button"
          className="font-archivo absolute right-2 top-1/2 -translate-y-1/2 px-6 text-lg bg-red-600 text-white hover:bg-red-700 disabled:bg-gray-300"
          onClick={search}
        >
          SZUKAJ
        </button>
        ...
        {city.name !== currentCitiesArray[0]?.name &&
          currentCitiesArray.length > 0 && (
            <ul className="absolute w-full max-h-[300px] overflow-y-auto top-[72px]">
