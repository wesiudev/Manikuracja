...
export default function Logic({ slugCity }: { slugCity?: string }) {
  ...
  return (
    <div className="flex flex-col gap-4 mt-3 w-full relative">
      <label
        htmlFor="city"
      ...
      <div className="flex flex-row">
        <input
          ...
          className="bg-black/50 text-lg placeholder:text-white z-[91] block sm:max-w-1/2 px-2 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-400"
          autoComplete="off"
        ...
        <button
          type="button"
          className="font-archivo z-[50] w-max max-w-full px-6 text-lg bg-red-600 text-white hover:bg-red-700 disabled:bg-gray-300"
          onClick={search}
        >
          SZUKAJ
        </button>
      ...
      {city.name !== currentCitiesArray[0]?.name &&
        currentCitiesArray.length > 0 && (
          <ul className="z-[60] absolute w-full max-h-[300px] overflow-y-auto top-12 shadow-lg rounded-md">
