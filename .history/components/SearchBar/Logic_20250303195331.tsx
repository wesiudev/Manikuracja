...
import ManicureSalons from "../ManicureSalons";
...
export default function Logic({ slugCity }: { slugCity?: string }) {
  ...
  const [locationDetails, setLocationDetails] = useState({ lng: 0, lat: 0 });
  ...
  const search = () => {
    if (city.name.length > 0) {
      router.push(`?manicure=${city.name}`);
    } else {
  ...
  return (
    <div className="flex flex-col mt-3 w-full relative">
      ...
      {locationDetails.lng !== 0 && (
        <ManicureSalons location={locationDetails} />
      )}
      {isMapOpen && (
        <div
